    ...
    if ( [data length] == 0 ) {
        ...
        if( success ) {
            if( CFHTTPMessageIsHeaderComplete(message) ) {
				...
				//subha mod.
				CFStringRef reqMethod = CFHTTPMessageCopyRequestMethod(message);
				if ([(NSString *)reqMethod isEqualToString:@"POST"]) {
					[delegate newPostReq:(NSData *)CFHTTPMessageCopyBody(message) connection:self];
				}
				else{
