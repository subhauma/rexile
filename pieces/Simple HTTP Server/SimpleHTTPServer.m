...
- (void)newPostReq:(NSData *)body connection:(SimpleHTTPConnection *)connection
{
	NSLog(@"inside newPostReq!");
	...
	NSLog( @"parse value is : %@ \n", (NSString *)([dict2 objectForKey:@"key1"]));
	
	//now create the response
	NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
						  @"postvalue1", @"key1", @"postvalue2", @"key2", nil];
	SBJsonWriter *writer = [[SBJsonWriter alloc] init];
	json_string = [writer stringWithObject:dict];
	body = [json_string dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
	
	//post out the response.
	CFHTTPMessageRef msg;
    msg = CFHTTPMessageCreateResponse(kCFAllocatorDefault,
                                      200,
                                      NULL, // Use standard status description 
                                      kCFHTTPVersion1_1);
	NSDictionary *headers = [NSDictionary dictionaryWithObject:@"String" forKey:@"Content-Type"];
    ...
    @catch (NSException *exception) {
        NSLog(@"Error while sending post response.");
    }
